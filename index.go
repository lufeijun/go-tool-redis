...
func main() {
	client, _err := CreateClient(
		common.String("12312"),
		common.String("21231312321"),
	)
	if _err != nil {
		fmt.Println("客户端连接失败：", _err)
	}

	// fmt.Println(common.Prettify(client))
	// return

	request := &cdn.RefreshObjectCachesRequest{
		ObjectPath: common.String("https://mallassets.zhufaner.com/6e141848-9abc-11ec-bac9-00163e352f67.jpg"),
	}

	_, err := client.RefreshObjectCaches(request)
	if err != nil {
		fmt.Println(common.Prettify(err))
	}
...
func CreateClient(accessKeyId *string, accessKeySecret *string) (_result *cdn.Client, _err error) {
	...
	config.Endpoint = common.String("cdn.aliyuncs.com")

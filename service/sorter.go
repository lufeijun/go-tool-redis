 ...
 */
func GetStringToSign(request *common.Request) (_result *string) {
	return common.String(getStringToSign(request))
}

func getStringToSign(request *common.Request) string {
	resource := common.StringValue(request.Pathname)
	...
	for i := 0; i < len(queryKeys); i++ {
		...
		v := common.StringValue(queryParams[queryKey])
		if v != "" {
			tmp = tmp + "&" + queryKey + "=" + v
		} else {
			tmp = tmp + "&" + queryKey
...
func getSignedStr(req *common.Request, canonicalizedResource string) string {
	temp := make(map[string]string)

	for k, v := range req.Headers {
		if strings.HasPrefix(strings.ToLower(k), "x-acs-") {
			temp[strings.ToLower(k)] = common.StringValue(v)
		}
	}
	hs := newSorter(temp)
	...
	date := common.StringValue(req.Headers["date"])
	accept := common.StringValue(req.Headers["accept"])
	contentType := common.StringValue(req.Headers["content-type"])
	contentMd5 := common.StringValue(req.Headers["content-md5"])

	signStr := common.StringValue(req.Method) + "\n" + accept + "\n" + contentMd5 + "\n" + contentType + "\n" + date + "\n" + canonicalizedOSSHeaders + canonicalizedResource
	return signStr
}
...
func GetROASignature(stringToSign *string, secret *string) (_result *string) {
	h := hmac.New(func() hash.Hash { return sha1.New() }, []byte(common.StringValue(secret)))
	io.WriteString(h, common.StringValue(stringToSign))
	signedStr := base64.StdEncoding.EncodeToString(h.Sum(nil))
	return common.String(signedStr)
}

func GetEndpoint(endpoint *string, server *bool, endpointType *string) *string {
	if common.StringValue(endpointType) == "internal" {
		strs := strings.Split(common.StringValue(endpoint), ".")
		strs[0] += "-internal"
		endpoint = common.String(strings.Join(strs, "."))
	}
	if common.BoolValue(server) && common.StringValue(endpointType) == "accelerate" {
		return common.String("oss-accelerate.aliyuncs.com")
	}
...
func GetEncodePath(path *string) *string {
	...
	return common.String(uri)
}

func GetEncodeParam(param *string) *string {
	uri := common.StringValue(param)
...
func GetAuthorization(request *common.Request, signatureAlgorithm, payload, acesskey, secret *string) *string {
	canonicalURI := common.StringValue(request.Pathname)
	if canonicalURI == "" {
		canonicalURI = "/"
	}
	...
	method := common.StringValue(request.Method)
	...
	canonicalRequest := method + "\n" + canonicalURI + "\n" + canonicalQueryString + "\n" + canonicalheaders + "\n" +
		strings.Join(signedHeaders, ";") + "\n" + common.StringValue(payload)
	signType := common.StringValue(signatureAlgorithm)
	StringToSign := signType + "\n" + common.StringValue(HexEncode(Hash([]byte(canonicalRequest), signatureAlgorithm)))
	signature := common.StringValue(HexEncode(SignatureMethod(common.StringValue(secret), StringToSign, signType)))
	auth := signType + " Credential=" + common.StringValue(acesskey) + ",SignedHeaders=" +
		strings.Join(signedHeaders, ";") + ",Signature=" + signature
	return common.String(auth)
}
...
func getCanonicalHeaders(headers map[string]*string) (string, []string) {
	...
	for k, v := range headers {
		if strings.HasPrefix(strings.ToLower(k), "x-acs-") || strings.ToLower(k) == "host" ||
			...
			tmp[strings.ToLower(k)] = strings.TrimSpace(common.StringValue(v))
			tmpHeader.Add(strings.ToLower(k), strings.TrimSpace(common.StringValue(v)))
		}
	}
	hs := newSorter(tmp)
...
func getCanonicalQueryString(query map[string]*string) string {
	canonicalQueryString := ""
	if common.BoolValue(common.IsUnset(query)) {
		return canonicalQueryString
	}
	tmp := make(map[string]string)
	for k, v := range query {
		tmp[k] = common.StringValue(v)
	}
...
func flatRepeatedList(dataValue reflect.Value, result map[string]*string, prefix string) {
	...
	if dataType.Kind().String() == "slice" {
		...
		result[prefix] = common.String(fmt.Sprintf("%v", dataValue.Interface()))
	}
}
	...
	if repeatedFieldValue.IsValid() && !repeatedFieldValue.IsNil() {
		for m := 0; m < repeatedFieldValue.Len(); m++ {
			...
			if fieldValue.Kind().String() == "map" {
				...
				result[key] = common.String(fmt.Sprintf("%v", fieldValue.Interface()))
			}
		}
	}
}
...
func GetTimestamp() (_result *string) {
	...
	return common.String(time.Now().In(gmt).Format("2006-01-02T15:04:05Z"))
}
...
func GetRPCSignature(signedParams map[string]*string, method *string, secret *string) (_result *string) {
	stringToSign := buildRpcStringToSign(signedParams, common.StringValue(method))
	signature := sign(stringToSign, common.StringValue(secret), "&")
	return common.String(signature)
}
...
func ArrayToStringWithSpecifiedStyle(array interface{}, prefix *string, style *string) (_result *string) {
	if common.BoolValue(common.IsUnset(array)) {
		return common.String("")
	}

	sty := common.StringValue(style)
	if sty == "repeatList" {
		tmp := map[string]interface{}{
			common.StringValue(prefix): array,
		}
...
func ParseToMap(in interface{}) map[string]interface{} {
	if common.BoolValue(common.IsUnset(in)) {
		return nil
	}
...
func buildRpcStringToSign(signedParam map[string]*string, method string) (stringToSign string) {
	...
	for key, value := range signedParam {
		signParams[key] = common.StringValue(value)
	}

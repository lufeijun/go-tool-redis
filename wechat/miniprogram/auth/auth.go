...
// NewAuth new auth
func NewAuth(ctx *wcontext.Context) *Auth {
	return &Auth{ctx}
}
...
func (auth *Auth) CheckEncryptedData(encryptedMsgHash string) (result RspCheckEncryptedData, err error) {
	return auth.CheckEncryptedDataContext(context.Background(), encryptedMsgHash)
}
...
func (auth *Auth) GetPhoneNumberContext(ctx context.Context, code string) (*GetPhoneNumberResponse, error) {
	var response []byte
	var (
		at  string
		err error
...
func (auth *Auth) GetPhoneNumber(code string) (*GetPhoneNumberResponse, error) {
	return auth.GetPhoneNumberContext(context.Background(), code)
}
